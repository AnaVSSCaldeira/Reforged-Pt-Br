...
local COASTAL_SHORE_OCEAN_COLOR = {
    primary_color        = {255,255,255,255},--{220, 240, 255,  60},
    secondary_color      = {0,0,0,0},--{ 21,  96, 110, 140},
...
local ocean_tile_spec_defaults = {
...
--]]
function AddTile(id, numerical_id, name, specs, minispecs, isflooring, isocean)
	...
	for k, default in pairs(isocean and ocean_tile_spec_defaults or tile_spec_defaults) do
		if specs[k] == nil then
	...
	if isocean then
		for i,j in pairs(real_specs) do
...
print("Changing ocean texture...")
for _,data in pairs(tiledefs.ground) do
	if data[1] == GROUND.OCEAN_COASTAL then
		print("- successfully changed ocean texture!")
		local CUSTOM_COLOR = {
		...
		data[2].noise_texture = "levels/textures/ground_water_coral.tex"
		data[2].colors = CUSTOM_COLOR
		break
	end
end
