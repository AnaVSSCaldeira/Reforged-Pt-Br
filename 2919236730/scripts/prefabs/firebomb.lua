...
local function AddChargeLvL(inst, level)
	if not inst.charge_fx then
		inst.charge_fx = COMMON_FNS.CreateFX("firebomb_sparks", nil, inst)
...
local function DoProcAlt(inst, caster, target)
	...
	local fx = COMMON_FNS.CreateFX("firebomb_proc_fx", target, caster)
...
--------------------------------------------------------------------------
return ForgePrefab("firebomb", fn, assets, prefabs, nil, tuning_values.ENTITY_TYPE, nil, "images/inventoryimages.xml", "lavaarena_firebomb.tex", "swap_lavaarena_firebomb", "common_hand"),
