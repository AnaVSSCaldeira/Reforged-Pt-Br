...
local function OnUpdateTargetFade(inst)
    local r,g,b,a = unpack(inst.colour)
    local k
    ...
    inst:Show() -- Forcing ret to show because something is hiding it, need to find out what is
    if inst._fade:value() == FADE_FRAMES then
...
local function OnColourDirty(inst)
    inst.colour = ConvertStringToTable(inst._colour:value())
    inst.AnimState:OverrideMultColour(unpack(inst.colour))
    if inst._fadetask == nil then
        inst._fadetask = inst:DoPeriodicTask(FRAMES, OnUpdateTargetFade)
    end
    inst:Show()
end

local function SetColour(inst, colour)
    inst._colour:set(SerializeTable(colour))
end
...
local function MakeTarget(name, anim, colour)
    local function OnTargetFadeDirty(inst)
        ...
        OnUpdateTargetFade(inst, unpack(inst.colour))
    end

    local function KillTarget(inst)
        if inst._fade:value() <= FADE_FRAMES then
            inst._fade:set(FADE_FRAMES + 1) -- increasing 1 higher makes it fade away
            if inst._fadetask == nil then
                inst._fadetask = inst:DoPeriodicTask(FRAMES, OnUpdateTargetFade)
            end
        end
    ...
    local function fn()
        ...
        --inst._fadetask = inst:DoPeriodicTask(FRAMES, OnUpdateTargetFade)
        inst._colour = net_string(inst.GUID, name.."._colour", "colourdirty")
...
return --MakeReticule("reticuleaoe", "idle"),
    ...
    MakeTarget("forge_reticule", "idle_target", { 0, 1, .25, 1 })
